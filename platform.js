...
async function join(){const c=code.value.trim();msg.textContent='';if(!/^\d{6}$/.test(c)){msg.textContent='الكود لازم يكون 6 أرقام.';return}try{await authReady();const snap=await get(ref(db,`rooms/${c}`));if(!snap.exists()){msg.textContent='الغرفة غير موجودة أو غير متاحة.';return}const r=snap.val(),type=r.gameType||'domino',routes={domino:'domino.html',chess:'chess.html',ludo:'ludo.html',backgammon:'backgammon.html',xo:'xo.html'};location.href=`./${routes[type]||'domino.html'}?room=${c}`;}catch(e){console.error(e);msg.textContent='تعذر قراءة الغرفة. جرّب من رابط اللعبة مباشرة.'}}
