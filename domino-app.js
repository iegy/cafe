...
import { firebaseConfig } from "./firebase-config.js?v=7.3.0";
import { turnConfig } from "./turn-config.js?v=7.3.0";
...
const els = {
  ...
  matchMode:$("matchMode"), targetScore:$("targetScore"), targetWrap:$("targetWrap"), rulesPreset:$("rulesPreset"), rulesPresetHint:$("rulesPresetHint"),
...
async function preloadSounds(){
  try{
    ...
    for(const [kind,url] of Object.entries({play:"./assets/domino-place-real.wav?v=7.3.0",double:"./assets/domino-double-real.wav?v=7.3.0",draw:"./assets/domino-draw-real.wav?v=7.3.0",win:"./assets/domino-win.wav?v=7.3.0"})){
...
function applyRulesPreset(preset,save=true){
  preset=preset||"standard";
  const standard=preset==="standard";
  if(standard){
    els.matchMode.value="points";
    els.targetScore.value="151";
    els.starterRule.value="winner";
    els.blockedScoring.value="loserTotal";
  }
  for(const el of [els.matchMode,els.targetScore,els.starterRule,els.blockedScoring]) if(el) el.disabled=standard;
  if(els.rulesPresetHint) els.rulesPresetHint.textContent=standard
    ?"7 قطع لكل لاعب، السحب حتى تظهر قطعة صالحة، الفائز يبدأ الجولة التالية، وعند القفل أقل مجموع يفوز ويأخذ مجموع نقاط الخصم."
    :"القواعد مخصصة: يمكنك تغيير الهدف والبداية وطريقة حساب القفل.";
...
async function createRoom(){
  ...
  saveProfile();
  localStorage.setItem("domino_rules_preset",els.rulesPreset?.value||"standard");
  ...
  for(let tries=0;tries<6;tries++){
    ...
    const data={
      hostUid:state.uid,status:"waiting",gameType:"domino",createdAt:Date.now(),
      players:{[state.uid]:{name:p.name,avatar:p.avatar,score:0,connected:true,lastSeen:Date.now()}},
      settings:{
        rulesPreset:els.rulesPreset?.value||"standard",
...
function renderLobby(room){
  ...
  const preset=room.settings?.rulesPreset||"custom";
  els.lobbyRules.textContent=`${preset==="standard"?"قواعد قياسية":"قواعد مخصصة"} • `+(mode==="single"?"جولة واحدة":`حتى ${target} نقطة`)+` • البداية: ${starterLabel} • القفل: ${blockLabel}`;
...
els.rulesPreset?.addEventListener("change",()=>applyRulesPreset(els.rulesPreset.value));
...
window.addEventListener("appinstalled",()=>{state.installPrompt=null;els.installAppBtn?.classList.add("hidden");toast("تم تثبيت القهوة ✅");});
...
(async function boot(){
  ...
  if(els.rulesPreset){els.rulesPreset.value=localStorage.getItem("domino_rules_preset")||"standard";applyRulesPreset(els.rulesPreset.value,false);}
  ...
  if("serviceWorker" in navigator)navigator.serviceWorker.register("./service-worker.js?v=7.3.0").catch(console.warn);
